...
class GetOsuMap(commands.Cog):
    ...
    async def getosumap(self, ctx, *, user: disnake.Member):
        ...
        if activities: 
            ...
            if not_osu_count == activity_count:
                await ctx.send(f"{user.name} isn't playing osu or isn't showing it on their activity. :(")
        else:
    ...
    async def getosumap2(self, ctx, user):
        ...
        user_id = response['id']
        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.get(f'{API_URL}/users/{user_id}/recent_activity') as resp:
    ...
    async def getosumap3(self, ctx, user):        
        ...
        maps = re.findall(r'(?<="url":").+?(?=",)|(?<=title":").+?(?=",)|(?<=artist":").+?(?=",)|(?<=ended_at":").+?(?=",)|(?<=pp":).+?(?=,)|(?<=accuracy":).+?(?=,)|(?<=rank":").+?(?=",)', maps[0])
